...
double obstacle_surface_distance(const Obstacle& obs, double* angle_out) {
    ...
    if (inside) {
        ...
        surface_dist = std::sqrt(local_dx * local_dx + local_dy * local_dy);
    }
...
void world_get_closest_obstacle(double* distance, double* angle) {
    ...
    for (const auto& obs : world_state.obstacles) {
        ...
        const double surface_dist = obstacle_surface_distance(obs, &obs_angle);
...
void world_get_obstacle_clearances(double* distances, size_t count) {
    ...
    for (const auto& obs : world_state.obstacles) {
        ...
        if (std::strstr(obs.name, "box") || std::strstr(obs.name, "cylinder")) {
            if (idx < count) {
                distances[idx] = obstacle_surface_distance(obs, nullptr);
            }
            ++idx;
        }
    }
    for (; idx < count; ++idx) {
        distances[idx] = std::numeric_limits<double>::quiet_NaN();
    }
}
...
double world_get_wall_clearance(void) {
    double best_distance = std::numeric_limits<double>::infinity();
    for (const auto& obs : world_state.obstacles) {
        if (!obs.name || !std::strstr(obs.name, "wall")) {
            continue;
        }
        const double surface_dist = obstacle_surface_distance(obs, nullptr);
        if (surface_dist < best_distance) {
            best_distance = surface_dist;
        }
    }
