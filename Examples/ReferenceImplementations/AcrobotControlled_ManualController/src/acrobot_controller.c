...
int acrobot_main(int argc, char* argv[]) {
    ...
    // Physical layout: World → BaseLink → ShoulderJoint → Link1 → ElbowJoint → Link2
    // ShoulderJoint = BASE joint (near fixed world) = Drake's q1
    // ElbowJoint = TIP joint (end of chain) = Drake's q2
    double shoulder_angle = 0.0;   // ShoulderEncoder.AngleOut = Drake's q1
    double shoulder_velocity = 0.0; // ShoulderEncoder.VelocityOut = Drake's dq1
    double elbow_angle = 0.0;      // ElbowEncoder.AngleOut = Drake's q2
    double elbow_velocity = 0.0;   // ElbowEncoder.VelocityOut = Drake's dq2
    ...
    for (;;) {
        ...
        // Physical layout matches Drake's convention directly:
        // - ShoulderEncoder = BASE joint = Drake's q1
        // - ElbowEncoder = TIP joint = Drake's q2
        // No swap needed!
        const double q1_raw = shoulder_angle;   // Shoulder = Drake's q1
        const double q2_raw = elbow_angle;      // Elbow = Drake's q2
        const double dq1 = shoulder_velocity;   // Shoulder velocity
        const double dq2 = elbow_velocity;      // Elbow velocity
