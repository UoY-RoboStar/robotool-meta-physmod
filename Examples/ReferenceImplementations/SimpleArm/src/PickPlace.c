...
#include <time.h>
#include <ctype.h>
// MANUAL: orchestrator bridge needs d-model interface API
...
M_PickPlace_output_Enum create_M_PickPlace_output_PrePick(float v1) {
	M_PickPlace_output_Data data;
...
M_PickPlace_output_Enum create_M_PickPlace_output_Return(float v1) {
	M_PickPlace_output_Data data;
...
M_PickPlace_output_Enum create_M_PickPlace_output_PrePlace(float v1) {
	M_PickPlace_output_Data data;
...
C_ctrl_ref0_output_Enum create_C_ctrl_ref0_output_Return(float v1) {
	C_ctrl_ref0_output_Data data;
...
C_ctrl_ref0_output_Enum create_C_ctrl_ref0_output_PrePick(float v1) {
	C_ctrl_ref0_output_Data data;
...
C_ctrl_ref0_output_Enum create_C_ctrl_ref0_output_PrePlace(float v1) {
	C_ctrl_ref0_output_Data data;
...
stm_ref0_output_Enum create_stm_ref0_output_PrePick(float v1) {
	stm_ref0_output_Data data;
...
stm_ref0_output_Enum create_stm_ref0_output_Return(float v1) {
	stm_ref0_output_Data data;
...
stm_ref0_output_Enum create_stm_ref0_output_PrePlace(float v1) {
	stm_ref0_output_Data data;
	...
	while (!terminate__) {
		{
			...
			while (!inputdone) {
				#pragma region MANUAL_orchestrator_input_interface
				// MANUAL: replace stdin prompt loop with orchestrator-driven input interface
				int input_type;
				EventData event;

				if (!registerRead(&input_type, &event, sizeof(event))) {
					...
					{
						char _s0[256];
						sprintf(_s0, "%s", "Engine terminated. Final cycle will run before this program ends.");
						printf("%s\n", _s0);
					}
					terminate__ = true;
					inputdone = true;
				...
				switch (input_type) {
					...
					{
						char _s0[256];
						sprintf(_s0, "%s%s", "Engine sent detectObject: ", event.occurred ? "true" : "false");
						printf("%s\n", _s0);
					}
					pthread_barrier_wait(&start_PickPlace->can_write);
					start_PickPlace->value = create_M_PickPlace_input_detectObject(event.occurred);
					pthread_barrier_wait(&start_PickPlace->can_read);
					break;
					...
					{
						char _s0[256];
						sprintf(_s0, "%s%s", "Engine sent detectGoal: ", event.occurred ? "true" : "false");
						printf("%s\n", _s0);
					}
					pthread_barrier_wait(&start_PickPlace->can_write);
					start_PickPlace->value = create_M_PickPlace_input_detectGoal(event.occurred);
					pthread_barrier_wait(&start_PickPlace->can_read);
					break;
					...
					{
						char _s0[256];
						sprintf(_s0, "%s", "Engine sent termination. Final cycle will run before this program ends.");
						printf("%s\n", _s0);
					}
					terminate__ = true;
					inputdone = true;
					...
					{
						char _s0[256];
						sprintf(_s0, "%s%d", "Unknown input type from engine: ", input_type);
						printf("%s\n", _s0);
					}
					break;
				}
				#pragma endregion MANUAL_orchestrator_input_interface
			}
		}
		{
			...
			while (!outputdone) {
				...
				if (_output_.type == M_PickPlace_output_PrePlace) {
					float _aux1_ = _output_.data.PrePlace.v1;	
					{
						char _s0[256];
					...
					}
					{
						...
						printf("%s\n", _s0);}
				     	...
				     	float _aux1_ = _output_.data.PrePick.v1;	
				     	{
				     		char _s0[256];
				     	...
				     	float _aux1_ = _output_.data.Return.v1;	
				     	{
				     		char _s0[256];
	...
	while (!terminate__) {
		{
			...
			while (!inputDone) {
				...
				if (_input_.type == M_PickPlace_input_detectObject) {
				...
				else if (_input_.type == M_PickPlace_input_detectGoal) {
		...
		mod_PickPlace_step(start_ctrl_ref0, end_ctrl_ref0);
		{
			...
			while (!outputDone) {
				...
				if (_output_.type == C_ctrl_ref0_output_PrePick) {
					...
					// MANUAL: inform orchestrator about platform operation and forward k
					registerWrite(&(OperationData){OUTPUT_PRE_PICK, 1, {(double)_aux1_}, 0.0});
				     ...
				     registerWrite(&(OperationData){OUTPUT_PRE_PLACE, 1, {(double)_aux1_}, 0.0});
				     	...
				     	outputDone = true;
				     // MANUAL: propagate Done handshake up to orchestrator
	...
	while (!terminate__) {
		...
		}
		{
			...
			while (!inputDone) {
				...
				if (_input_.type == C_ctrl_ref0_input_detectGoal) {
					bool _aux1_ = _input_.data.detectGoal.v1;	
					{
						pthread_barrier_wait(&start_stm_ref0->can_write);
				...
				else if (_input_.type == C_ctrl_ref0_input_detectObject) {
				     	bool _aux1_ = _input_.data.detectObject.v1;	
				     	{
				     		pthread_barrier_wait(&start_stm_ref0->can_write);
...
#pragma region MANUAL_orchestrator_main_interface
// MANUAL: expose pickplace_main so the orchestrator owns process entry and engine lifecycle
int pickplace_main(int argc, char* argv[]) {
    ...
    // MANUAL: replicate generator structs locally so we can serialize outputs without depending on generated headers
    typedef struct {
    float v1;
    } stm_ref0_output_PrePick_Data;
	...
	fclose(log_file);
	// MANUAL: return status to orchestrator so it can mirror PickPlace's exit code
	return 0;
}
#pragma endregion MANUAL_orchestrator_main_interface
	...
	RESULT_Enum tr_PickPlaceS_t2(struct stm_ref0_state* state, struct stm_ref0_inputstate* inputstate, struct stm_ref0_memory* memory, stm_ref0_output_Enum_Channel* output) {
		...
		} else if (	(state)->tr_PickPlaceS_t2_counter == 1 ) {
			(*state).tr_PickPlaceS_t2_counter = 2;
	...
	RESULT_Enum tr_PickPlaceS_t7(struct stm_ref0_state* state, struct stm_ref0_inputstate* inputstate, struct stm_ref0_memory* memory, stm_ref0_output_Enum_Channel* output) {
		...
		if (	(state)->tr_PickPlaceS_t7_counter == 0 ) {
		...
		} else if (	(state)->tr_PickPlaceS_t7_counter == 2 ) {
			{
				...
				output->value = create_stm_ref0_output_PrePlace((memory)->k);
	...
	RESULT_Enum tr_PickPlaceS_t9(struct stm_ref0_state* state, struct stm_ref0_inputstate* inputstate, struct stm_ref0_memory* memory, stm_ref0_output_Enum_Channel* output) {
		...
		if (	(state)->tr_PickPlaceS_t9_counter == 0 ) {
			...
			{
				pthread_barrier_wait(&output->can_write);
				output->value = create_stm_ref0_output_Return((memory)->k);
				pthread_barrier_wait(&output->can_read);
			}
	...
	RESULT_Enum tr_PickPlaceS_t0(struct stm_ref0_state* state, struct stm_ref0_inputstate* inputstate, struct stm_ref0_memory* memory, stm_ref0_output_Enum_Channel* output) {
		...
		if (	(state)->tr_PickPlaceS_t0_counter == 0 ) {
			...
			return create_RESULT_CONT();
		} else if (	(state)->tr_PickPlaceS_t0_counter == 1 ) {
			(*state).tr_PickPlaceS_t0_counter = 2;
			return create_RESULT_WAIT();
		} else {
	...
	RESULT_Enum tr_PickPlaceS_t6(struct stm_ref0_state* state, struct stm_ref0_inputstate* inputstate, struct stm_ref0_memory* memory, stm_ref0_output_Enum_Channel* output) {
		...
		if (	(state)->tr_PickPlaceS_t6_counter == 0 ) {
			...
			{
				pthread_barrier_wait(&output->can_write);
				output->value = create_stm_ref0_output_PrePick((memory)->k);
				pthread_barrier_wait(&output->can_read);
			}
		     	...
		     	if ((*state).status.type == create_STATUS_ENTER_STATE().type) {
		     		...
		     		}
		     		{
		     			...
		     			return create_RESULT_CONT();
		     	     	...
		     	     	{
		     	     		if (	(*inputstate)._transition_.type == create_TRANSITIONS_stm_ref0_stm_ref0_t7().type
		     	     		     ) {
		     	     		...
		     	     		} else if (	(*inputstate)._transition_.type == create_TRANSITIONS_stm_ref0_stm_ref0_t0().type
		     	     		            ) {
		     	...
		     	if ((*state).status.type == create_STATUS_ENTER_STATE().type) {
		     		...
		     		}
		     		{
		     			...
		     			return create_RESULT_CONT();
		     	     	...
		     	     	{
		     	     		if (	(*inputstate)._transition_.type == create_TRANSITIONS_stm_ref0_stm_ref0_t5().type
		     	     		     ) {
		     	     		...
		     	     		} else if (	(*inputstate)._transition_.type == create_TRANSITIONS_stm_ref0_stm_ref0_t18().type
		     	     		            ) {
		     	     			(*state).state = create_STATES_stm_ref0_Finding_Goal();
		     	     			(*state).status = create_STATUS_ENTER_STATE();
		     	     			return create_RESULT_CONT();
