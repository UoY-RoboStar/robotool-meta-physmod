...
/**
 * LaTeX generation integration test for Acrobot SKO:
 * - Generates LaTeX document from acrobot_gravity_only.slnRef
...
public class TestSKO_Acrobot_Latex {
    ...
    private static final String TESTDATA_PROJECT_PATH =
        "../../physmod-testdata/circus.robocalc.robosim.physmod.testdata/testdata/integration/T5/Acrobot/SKO";
    ...
    private Path acrobotSkoRoot() {
        return Paths.get("").toAbsolutePath().resolve(TESTDATA_PROJECT_PATH);
    }

    @Test
    public void testAcrobotLatexGeneration() throws Exception {
        // Use input from acrobot SKO folder
        Path input = acrobotSkoRoot().resolve("standalone_visualisation").resolve("input").resolve("acrobot_gravity_only.slnRef");
        ...
        try {
            ...
            // Write intermediate .sln file to temp directory
            Path outDir = acrobotSkoRoot().resolve("latex").resolve("temp");
            Files.createDirectories(outDir);
            Path slnFile = outDir.resolve("solution.sln");
            Files.writeString(slnFile, solutionDSL);
            System.out.println("Intermediate .sln output: " + slnFile);
            ...
            Path expectedLatex = acrobotSkoRoot().resolve("latex").resolve("expected").resolve("solution.tex");
