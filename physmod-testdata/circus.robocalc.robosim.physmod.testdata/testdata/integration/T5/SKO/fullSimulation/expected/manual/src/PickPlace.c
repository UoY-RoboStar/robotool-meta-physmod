	...
	while (!terminate__) {
		{
			...
			while (!inputdone) {
				...
				switch (input_type) {
					...
					{
						char _s0[256];
						sprintf(_s0, "%s%s", "Engine sent detectObject: ", input_value.occurred ? "true" : "false");
						printf("%s\n", _s0);
					}
					pthread_barrier_wait(&start_PickPlace->can_write);
					start_PickPlace->value = create_M_PickPlace_input_detectObject(input_value.occurred);
					pthread_barrier_wait(&start_PickPlace->can_read);
					break;
					...
					{
						char _s0[256];
						sprintf(_s0, "%s%s", "Engine sent detectGoal: ", input_value.occurred ? "true" : "false");
						printf("%s\n", _s0);
					}
					pthread_barrier_wait(&start_PickPlace->can_write);
					start_PickPlace->value = create_M_PickPlace_input_detectGoal(input_value.occurred);
					pthread_barrier_wait(&start_PickPlace->can_read);
					break;
