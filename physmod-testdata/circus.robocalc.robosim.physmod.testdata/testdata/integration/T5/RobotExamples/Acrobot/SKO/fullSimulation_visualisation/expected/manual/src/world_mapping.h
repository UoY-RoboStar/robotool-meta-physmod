...
static inline void scenario_update_sensors(const std::vector<Eigen::MatrixXd>& Bk,
    ...
    // Beam sensor: reports object/goal Y position (matching manualImplementation behavior)
    // Detection occurs immediately since object_y=4.7511 is in range [4.749, 4.75111]
    // This is correct: d-model cycles through operations with proper timing
    if (object_sensor) {
        *object_sensor = fabs(object_y);
    }

    if (goal_sensor) {
        *goal_sensor = fabs(goal_y);
    }
}
