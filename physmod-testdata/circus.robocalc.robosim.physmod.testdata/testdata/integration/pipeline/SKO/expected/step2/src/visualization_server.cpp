...
private:
    ...
    void updateTransform(const TransformData* data) {
        ...
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                transform(i, j) = data->transform[i * 4 + j];
            }
        }
    ...
    void createObject(const ObjectData* data) {
        ...
        switch (data->shape_type) {
            ...
            case 3: {
                meshcat_->set_object(data->object_name,
                    MeshcatCpp::Mesh(std::string(data->mesh_path)),
                    material);
                break;
            }
        }
...
static VisualizationServer g_server;

extern "C" {

int visualization_server_start(int port) {
    return g_server.start(port) ? 0 : 1;
}

void visualization_server_stop() {
    g_server.stop();
}
