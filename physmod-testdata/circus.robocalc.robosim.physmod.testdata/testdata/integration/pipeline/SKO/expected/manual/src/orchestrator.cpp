...
extern "C" int pickplace_main(int argc, char* argv[]);
...
static mapping_config_t mappingConfig = DEFAULT_MAPPING_CONFIG;
 ...
 */
 static void dmodel_publish_input_message(void) {
    ...
    // Derive boolean events from platform mapping sensor registers (paper-aligned)
    const double obj_meas = p_mapping.SimpleArm.BaseLink.ObjectBeamSensor.measurement;
    const double goal_meas = p_mapping.SimpleArm.BaseLink.GoalBeamSensor.measurement;
    inputRegister.fields.event1 = Event1(obj_meas, &mappingConfig);
    inputRegister.fields.event2 = Event2(goal_meas, &mappingConfig);
...
static inline void populate_input_buffer_locked(const sensor_outputs_t* outputs) {
    p_mapping.SimpleArm.BaseLink.ObjectBeamSensor.measurement =
        outputs->SimpleArm.BaseLink.ObjectBeamSensor.measurement;
    p_mapping.SimpleArm.BaseLink.GoalBeamSensor.measurement =
        outputs->SimpleArm.BaseLink.GoalBeamSensor.measurement;

    inputRegister.fields.event1 = Event1(outputs->SimpleArm.BaseLink.ObjectBeamSensor.measurement, &mappingConfig);
    inputRegister.fields.event2 = Event2(outputs->SimpleArm.BaseLink.GoalBeamSensor.measurement, &mappingConfig);
    inputRegister.pending.event1Pending = true;
    inputRegister.pending.event2Pending = true;
    inputRegister.doneReady = false;
    inputRegister.ready = true;
...
static bool registerRead_impl(int* type, void* data, size_t size) {
    if (type == nullptr || data == nullptr || size < sizeof(EventData)) {
        return false;
    }
...
static void SendToDModel_impl(const sensor_outputs_t* outputs) {
    if (outputs == nullptr) {
        return;
    }
    ...
    populate_input_buffer_locked(outputs);
...
int main(int argc, char* argv[]) {
    ...
    int pickplace_exit_code = pickplace_main(argc, argv);
