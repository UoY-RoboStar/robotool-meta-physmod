...
// Sensor event queue for Acrobot - delivers sensor values via registerRead
// Only robot sensors (from platform mapping) are delivered, not world properties
typedef struct {
    bool ready;           // Buffer has been populated for this cycle
    bool doneReady;       // INPUT_DONE ready to deliver
    
    // Sensor values read from p_mapping (robot sensors only)
    struct {
    ...
    // Pending flags for each sensor event
    struct {
        bool shoulderAnglePending;
        bool shoulderVelocityPending;
        bool elbowAnglePending;
        bool elbowVelocityPending;
    } pending;
} input_register_t;
 ...
 * The controller receives each sensor as a separate INPUT_* event, followed by INPUT_DONE.
 */
static void dmodel_publish_input_message(void) {
    ...
    // Read sensor values from p_mapping (robot sensors only, not world properties)
    inputRegister.sensors.shoulder_angle = p_mapping.AcrobotControlled.BaseLink.ShoulderEncoder.AngleOut;
    inputRegister.sensors.shoulder_velocity = p_mapping.AcrobotControlled.BaseLink.ShoulderEncoder.VelocityOut;
    inputRegister.sensors.elbow_angle = p_mapping.AcrobotControlled.Link1.ElbowEncoder.AngleOut;
    inputRegister.sensors.elbow_velocity = p_mapping.AcrobotControlled.Link1.ElbowEncoder.VelocityOut;
    
    // Mark all sensor events as pending
    inputRegister.pending.shoulderAnglePending = true;
    inputRegister.pending.shoulderVelocityPending = true;
    inputRegister.pending.elbowAnglePending = true;
    inputRegister.pending.elbowVelocityPending = true;
...
/**
 * Deliver the next buffered sensor event to the controller.
 * Events are delivered in order: shoulder_angle, shoulder_velocity, elbow_angle, elbow_velocity, then DONE.
 * Note: Only robot sensors (from platform mapping) are delivered, not world properties.
 */
static inline bool deliver_buffered_event_locked(int* type, void* data, size_t size) {
    EventData* event = nullptr;
    if (data != nullptr && size >= sizeof(EventData)) {
        event = static_cast<EventData*>(data);
    }
    // Deliver sensor events in order
    if (inputRegister.pending.shoulderAnglePending) {
        *type = INPUT_SHOULDER_ANGLE;
        if (event) {
            event->occurred = true;
            event->value = inputRegister.sensors.shoulder_angle;
        }
        inputRegister.pending.shoulderAnglePending = false;
        return true;
    }
    if (inputRegister.pending.shoulderVelocityPending) {
        *type = INPUT_SHOULDER_VELOCITY;
        if (event) {
            event->occurred = true;
            event->value = inputRegister.sensors.shoulder_velocity;
        }
        inputRegister.pending.shoulderVelocityPending = false;
        return true;
    }
    if (inputRegister.pending.elbowAnglePending) {
        *type = INPUT_ELBOW_ANGLE;
        if (event) {
            event->occurred = true;
            event->value = inputRegister.sensors.elbow_angle;
        }
        inputRegister.pending.elbowAnglePending = false;
        return true;
    }
    if (inputRegister.pending.elbowVelocityPending) {
        *type = INPUT_ELBOW_VELOCITY;
        if (event) {
            event->occurred = true;
            event->value = inputRegister.sensors.elbow_velocity;
        }
        inputRegister.pending.elbowVelocityPending = false;
        // After elbow velocity, mark DONE as ready
        inputRegister.doneReady = true;
        return true;
    }
...
static void EvolveWorldMapping() {
    ...
    // Copy sensor outputs into platform mapping via the adapter
...
void* engine_thread(void* arg) {
    ...
    while (!engine_input.terminate) {
        ...
        if (cycle_completed) {
            ...
            // 2. Advance d-model with INPUT_DONE
            tock(INPUT_DONE);

            // 3. Wait for controller output and apply immediately (lockstep).
