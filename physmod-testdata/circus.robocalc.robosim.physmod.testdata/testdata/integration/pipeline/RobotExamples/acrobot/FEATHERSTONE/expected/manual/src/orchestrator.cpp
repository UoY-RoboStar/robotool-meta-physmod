...
static void EvolveWorldMapping() {
    ...
    if (st.theta.size() >= 1) p_mapping.Acrobot.BaseLink.ShoulderEncoder.AngleOut = st.theta(0);
    if (st.d_theta.size() >= 1) p_mapping.Acrobot.BaseLink.ShoulderEncoder.VelocityOut = st.d_theta(0);
    if (st.theta.size() >= 2) p_mapping.Acrobot.Link1.ElbowEncoder.AngleOut = st.theta(1);
    if (st.d_theta.size() >= 2) p_mapping.Acrobot.Link1.ElbowEncoder.VelocityOut = st.d_theta(1);
}
